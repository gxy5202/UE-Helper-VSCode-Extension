...
const fs = require('fs');
const { FIRST_LEVEL, SECOND_LEVEL, THIRD_LEVEL, FOURTH_LEVEL, RANGE, UI_TYPE, VALUE } = configMap_1.HEADER;
const settingGenerator = {
    ...
    setChildren: (obj, arr) => {
        Object.keys(obj).forEach((key) => {
            ...
            switch (item["level"]) {
                ...
                case FIRST_LEVEL:
                    if (arr.length === 0) {
                        arr.push(JSON.parse(JSON.stringify(configMap_1.UI_OPTIONS['tabs'])));
                    }
                    child = {
                        label: key,
                        children: []
                    ...
                    // 系列样式特殊处理
                    if (key === '系列') {
                        child.children.push(configMap_1.UI_OPTIONS['editableTabs']);
                    }
                    arr[0].children.push(child);
                    break;
                // 一级分类使用group
                case SECOND_LEVEL:
                    child = JSON.parse(JSON.stringify(configMap_1.UI_OPTIONS['group']));
                    child['label'] = key;
                    arr.push(child);
                    break;
                // 二级分类使用row
                case THIRD_LEVEL:
                    child = JSON.parse(JSON.stringify(configMap_1.UI_OPTIONS['row']));
                    child['label'] = key;
                    ...
                    if (rowKeys.length > 0) {
                        rowKeys.forEach((item) => {
                            const row = JSON.parse(JSON.stringify(configMap_1.UI_OPTIONS['row']));
                            row.rowId = item;
                            row.showLabel = false;
                            delete row.label;
                ...
                // 配置项使用各自控件
                case FOURTH_LEVEL:
                    child = utils_1.setUIOptionsRange(item[UI_TYPE], item, key);
                    child['label'] = key;
    ...
    changeSpecialOptionContent: (item) => {
        switch (item.type) {
            case 'editableTabs': {
    ...
    generateSettings: (path) => {
        const sheetJson = parse_1.default.convertToJson(path);
        const publicConfig = parse_1.default.convertToJson(path, true);
        const treeObj = {};
        parse_1.default.convertToTree(sheetJson, utils_1.convertPublicConfig(publicConfig), treeObj);
        ...
        fs.writeFile(`${path}/setting.ts`, stringJson, (err) => {
            console.log(err);
        });
    }
};
exports.default = settingGenerator;
