 ...
 * @param {*} newTree 
 */
function compareMergeNode(targetNode: any, newNode: any) {
    if (!newNode.children) {
        return;
    }
    ...
    const targetLeaf = targetNode.children.find((item: any) => item.label === label);
 ...
 * @returns 
 */
function parseWarningRenderJson2Tree(json: any, commonPropsJson: any) {
    const newJson: any = {};
 ...
 * @param {*} json 
 */
function parseWarningRenderTree2Settings(jsonObject: any) {
    const settings: any[] = [];
